...
import { DesignarEntidadeModal } from "@/components/DesignarEntidadeModal";
...
export default function ProjetosSubmetidos() {
    ...
    const handleDesignarEnts = (codProjeto: number) => {
    ...
    const handleConfirmDesignacao = async (codProjeto: number, codEntExec: number, codEntGer: number) => {
        try {
            await api.put(`/projetos/submetidos/${codProjeto}`, {
                CodEntExec: codEntExec,
                codEntGer,
            });
            toast.success("Entidades designadas com sucesso!");
            fetchProjetosSubmetidos();
            handleCloseModal();
        } catch (error) {
            console.error("Erro ao designar entidades:", error);
            toast.error("Erro ao designar entidades.");
        }
    };
    ...
    return (
        <div className="w-full mx-auto bg-white p-6 rounded-2xl shadow-lg">
                ...
                <div className="w-full flex flex-col p-4">
                    {projetosFiltrados.map((projeto) => (
                        <ProjetoCard
                            ...
                            projeto={projeto}
                            onDesignar={() => handleDesignarEnts(projeto.codProjeto)}
                            showDesignar
                        />
                    ))}
                </div>
            ...
            {selectedProjeto && (
                <DesignarEntidadeModal
                    isOpen={isModalOpen}
                    onClose={handleCloseModal}
                    codProjeto={selectedProjeto}
                    onConfirm={handleConfirmDesignacao}
                />
            )}
        </div>
    )
